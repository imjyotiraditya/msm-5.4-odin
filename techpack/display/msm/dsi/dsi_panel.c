				      ...
				      int bl_lvl)
{
	...
	case DSI_CMD_SET_MI_LOCAL_HBM_NORMAL_WHITE_1000NIT:
		index = panel->local_hbm_on_87_index;
		break;
	default:
...
static int dsi_panel_parse_fod(struct dsi_panel *panel)
{
	...
	panel->local_hbm_on_87_index = -1;
	rc = utils->read_u32(utils->data, "mi,local-hbm-on-87-index",
			     &panel->local_hbm_on_87_index);
	if (rc) {
		DSI_INFO("mi,local-hbm-on-87-index not specified\n");
	} else {
		DSI_INFO("mi,local-hbm-on-87-index is %d\n", panel->local_hbm_on_87_index);
	}
